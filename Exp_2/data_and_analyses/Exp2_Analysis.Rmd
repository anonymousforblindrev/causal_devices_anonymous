...
bar.summary+ 
  ...
  #geom_bar(stat="identity",position="dodge",colour="black", fill = c("#ffe6e6", "#ceeef9", "#defcce"))+
  geom_bar(stat="identity",position="dodge",colour="black", fill = "lightgrey")+
  ...
  geom_text(aes(y=.04,label=n),colour="black",size=8)+
...
ggsave("01_results.png",height=3,width=3)
```
...
bar.conf+ 
  ...
  geom_errorbar(mapping=aes(ymin=LB.95,ymax=UB.95),stat="identity",position=position_dodge(width=.9),width=.2)+
  geom_text(aes(y=1.5,label=n),colour="black",position=position_dodge(width=.9),size=8)+
  # ggtitle("Results")+
  myTheme+
  # ggtitle("Results")+
  theme(plot.title=element_text(face="bold",size=12),
...
ggsave("01_confidence.png",height=3,width=5)
```
...
a.conf <- aov_car(value ~ cond*variable + Error(sID/variable), data.conf, anova_table = list(es = "pes"))
a.conf

```


## Descriptive stats for the confidence ratings
...
means <- lsmeans(a.conf, c("cond", "variable")) 
means
```
